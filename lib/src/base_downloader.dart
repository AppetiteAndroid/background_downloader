...
abstract base class BaseDownloader {
    ...
    final instance = Platform.isAndroid
        ...
        : Platform.isIOS
            ...
            : Platform.isLinux || Platform.isMacOS || Platform.isWindows
                ? DesktopDownloader()
                : throw ArgumentError('${Platform.operatingSystem} is not a supported platform');
  ...
  Future<List<(String, String)>> configure({dynamic globalConfig, dynamic androidConfig, dynamic iOSConfig, dynamic desktopConfig}) async {
  ...
  Future<TaskStatusUpdate> enqueueAndAwait(Task task,
    ...
    final requiredUpdates = onProgress != null || taskProgressCallback != null ? Updates.statusAndProgress : Updates.status;
    ...
    if (!enqueueSuccess) {
      ...
      return Future.value(TaskStatusUpdate(taskToEnqueue, TaskStatus.failed, TaskException('Could not enqueue task $taskToEnqueue')));
    }
    if (timer != null) {
  ...
  Future<Batch> enqueueAndAwaitBatch(final List<Task> tasks,
    ...
    for (final task in tasks) {
      taskFutures.add(enqueueAndAwait(task, taskStatusCallback: taskStatusCallback, taskProgressCallback: taskProgressCallback));
      if (counter++ % 3 == 0) {
  ...
  Future<bool> cancelTasksWithIds(List<String> taskIds) async {
    final matchingTasksWaitingToRetry = tasksWaitingToRetry.where((task) => taskIds.contains(task.taskId)).toList(growable: false);
    final matchingTaskIdsWaitingToRetry = matchingTasksWaitingToRetry.map((task) => task.taskId).toList(growable: false);
    ...
    // cancel paused tasks
    final pausedTasks = await getPausedTasks();
    final pausedTaskIdsToCancel = pausedTasks.where((task) => remainingTaskIds.contains(task.taskId)).map((e) => e.taskId).toList(growable: false);
    await cancelPausedPlatformTasksWithIds(pausedTasks, pausedTaskIdsToCancel);
    // cancel remaining taskIds on the platform
    final platformTaskIds = remainingTaskIds.where((taskId) => !pausedTaskIdsToCancel.contains(taskId)).toList(growable: false);
        ...
        if (resumeData != null) {
          if (task is ParallelDownloadTask) {
            ...
            for (final chunk in chunks) {
              final tempFilePath = (await getResumeData(chunk.task.taskId))?.tempFilepath;
  ...
  Future<void> trackTasks(String? group, bool markDownloadedComplete) async {
    ...
    if (markDownloadedComplete) {
      ...
      for (var record in records.where((record) => record.task is DownloadTask && record.status != TaskStatus.complete)) {
      ...
      Future.delayed(waitTime, () async {
        ...
        if (tasksWaitingToRetry.remove(task)) {
          if (!((await getResumeData(task.taskId) != null && await resume(task)) || await enqueue(task))) {
            ...
            _emitStatusUpdate(TaskStatusUpdate(task, TaskStatus.failed, TaskException('Could not resume/enqueue taskId${task.taskId} after retry timeout')));
  ...
  Future<void> _updateTaskInDatabase(Task task, {TaskStatus? status, double? progress, int expectedFileSize = -1, TaskException? taskException}) async {
    if (trackedGroups.contains(null) || trackedGroups.contains(task.group)) {
      ...
      if (status != TaskStatus.paused) {
        ...
        final existingRecord = await database.recordForId(task.taskId);
        database.updateRecord(TaskRecord(task, status!, existingRecord?.progress ?? 0, expectedFileSize, taskException));
      }
    }
  }
